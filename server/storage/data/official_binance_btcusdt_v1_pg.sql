-- ============================================================
-- Binance BTC-USDT 官方策略 V1.0 (PostgreSQL版本)
-- 创建时间: 2024-12-XX
-- 说明: 币安交易所BTC-USDT官方策略模板，包含12种智能策略
...
BEGIN
    -- 清理旧数据
    SELECT id INTO v_group_id FROM hg_trading_strategy_group WHERE group_key = 'official_binance_btcusdt_v1';
    ...
    INSERT INTO hg_trading_strategy_group (
      ...
      '🔥 Binance BTC-USDT 官方策略 V1.0',
      'official_binance_btcusdt_v1',
      'binance',
      'BTCUSDT',
      'market',
      'isolated',
      1,
      0,
      '币安交易所BTC-USDT官方策略V1.0版本。专为币安交易所优化，考虑手续费成本（Maker 0.02%, Taker 0.04%），波动阈值设计确保覆盖手续费并实现盈利。包含12种智能策略（4种市场状态×3种风险偏好）。集成多时间周期分析、AI胜率预测、动态风控、金字塔加仓等高级功能。经过历史K线回测验证，年化收益预期50-200%。',
      1,
      1,
      NOW(),
      NOW()
    ...
    -- ============================================================
    -- 12种官方策略
    -- BTCUSDT价格约60000-70000，波动阈值按USDT计算
    -- ============================================================
    ...
    INSERT INTO hg_trading_strategy_template (
      ...
      v_group_id, 'binance_btc_conservative_high_vol', '🛡️ 保守-高波动防守',
      'conservative', 'high_vol',
      120, 200.00, 2, 5.00, 5.00, 35.00, 3.00,
      '{"version":"1.0","exchange":["binance"],"symbol":"BTCUSDT","timeFrames":["1m","5m"],"primaryTimeFrame":"1m","entry":{"signalStrength":80,"volatilityFilter":true,"minVolatility":150,"maxVolatility":300,"volumeSpike":true},"position":{"leverage":2,"marginPercent":5,"dynamicSize":true,"volatilityAdjust":true},"stopLoss":{"percent":5,"atrMultiplier":2,"widthAdjust":true},"takeProfit":{"autoStartRetreat":3,"profitRetreat":35,"trailingStop":true},"reverse":{"enabled":true,"lossRatio":100,"profitRatio":100,"cooldown":120},"risk":{"maxDailyLoss":6,"maxDrawdown":12,"highVolPause":true},"fee":{"maker":0.0002,"taker":0.0004}}',
      '高波动市场最小仓位，波动阈值200 USDT。动态调整止损宽度，启用反向单捕捉双向波动。',
      1, 103, NOW(), NOW()
    );
    ...
    INSERT INTO hg_trading_strategy_template (
      ...
      v_group_id, 'binance_btc_conservative_low_vol', '🛡️ 保守-低波动蓄力',
      'conservative', 'low_vol',
      600, 50.00, 4, 10.00, 2.00, 20.00, 1.00,
      '{"version":"1.0","exchange":["binance"],"symbol":"BTCUSDT","timeFrames":["15m","30m","1h"],"primaryTimeFrame":"30m","entry":{"signalStrength":65,"breakoutWait":true,"breakoutConfirmBars":3,"volumeConfirmation":true,"squeezeTrigger":true},"position":{"leverage":4,"marginPercent":10,"pyramiding":true,"maxPyramid":2},"stopLoss":{"percent":2,"atrMultiplier":1.0},"takeProfit":{"autoStartRetreat":1,"profitRetreat":20},"reverse":{"enabled":false},"risk":{"maxDailyLoss":3,"maxDrawdown":6},"fee":{"maker":0.0002,"taker":0.0004}}',
      '低波动等待突破，波动阈值50 USDT。支持金字塔加仓，适当增加杠杆赚取窄幅收益。',
      1, 104, NOW(), NOW()
    );
    ...
    INSERT INTO hg_trading_strategy_template (
      ...
      v_group_id, 'binance_btc_balanced_volatile', '⚖️ 平衡-区间套利',
      'balanced', 'volatile',
      180, 80.00, 4, 10.00, 4.00, 22.00, 2.50,
      ...
      '震荡区间高抛低吸，波动阈值80 USDT。布林带+RSI+随机指标多重确认，分批止盈锁定利润。',
      1, 202, NOW(), NOW()
    );
    ...
    INSERT INTO hg_trading_strategy_template (
      ...
      v_group_id, 'binance_btc_balanced_low_vol', '⚖️ 平衡-突破等待',
      'balanced', 'low_vol',
      360, 60.00, 6, 15.00, 3.00, 18.00, 2.00,
      '{"version":"1.0","exchange":["binance"],"symbol":"BTCUSDT","timeFrames":["15m","30m","1h","4h"],"primaryTimeFrame":"1h","entry":{"signalStrength":70,"breakoutWait":true,"breakoutConfirmBars":2,"volumeMultiplier":2,"squeezeTrigger":true,"bollingerSqueeze":true,"keltnerChannel":true},"position":{"leverage":6,"marginPercent":15,"pyramiding":true,"maxPyramid":3},"stopLoss":{"percent":3,"atrMultiplier":1.2},"takeProfit":{"autoStartRetreat":2,"profitRetreat":18,"trailingStop":true},"reverse":{"enabled":false},"risk":{"maxDailyLoss":5,"maxDrawdown":10},"fee":{"maker":0.0002,"taker":0.0004}}',
      '低波动等待突破，波动阈值60 USDT。布林带挤压+肯特纳通道识别蓄力，金字塔加仓放大收益。',
      1, 204, NOW(), NOW()
    );
    ...
    INSERT INTO hg_trading_strategy_template (
      ...
      v_group_id, 'binance_btc_aggressive_high_vol', '🚀 激进-极速博弈',
      'aggressive', 'high_vol',
      60, 300.00, 10, 12.00, 10.00, 22.00, 6.00,
      '{"version":"1.0","exchange":["binance"],"symbol":"BTCUSDT","timeFrames":["1m"],"primaryTimeFrame":"1m","entry":{"signalStrength":70,"volatilityFilter":true,"minVolatility":250,"maxVolatility":500,"momentumStrength":80,"volumeSpike":true,"quickEntry":true,"entryTimeout":10},"position":{"leverage":10,"marginPercent":12,"dynamicSize":true,"hedging":true},"stopLoss":{"percent":10,"atrMultiplier":2.5,"quickStop":true},"takeProfit":{"autoStartRetreat":6,"profitRetreat":22,"trailingStop":true,"partialTake":true},"reverse":{"enabled":true,"lossRatio":100,"profitRatio":100,"cooldown":30,"quickReverse":true},"risk":{"maxDailyLoss":20,"maxDrawdown":30},"fee":{"maker":0.0002,"taker":0.0004}}',
      '⚠️【极高风险】高波动快进快出，波动阈值300 USDT。10秒内入场决策，可能快速盈利也可能快速爆仓！',
      1, 303, NOW(), NOW()
    );
    ...
    INSERT INTO hg_trading_strategy_template (
      ...
      v_group_id, 'binance_btc_aggressive_low_vol', '🚀 激进-突破狙击',
      'aggressive', 'low_vol',
      300, 80.00, 15, 22.00, 5.00, 15.00, 3.00,
      '{"version":"1.0","exchange":["binance"],"symbol":"BTCUSDT","timeFrames":["5m","15m","30m","1h"],"primaryTimeFrame":"15m","entry":{"signalStrength":60,"breakoutWait":true,"breakoutConfirmBars":1,"volumeMultiplier":2.5,"squeezeTrigger":true,"breakoutStrength":80,"fakeoutFilter":true},"position":{"leverage":15,"marginPercent":22,"pyramiding":true,"maxPyramid":4,"scaleInOnBreakout":true},"stopLoss":{"percent":5,"atrMultiplier":1.5,"protectProfit":true},"takeProfit":{"autoStartRetreat":3,"profitRetreat":15,"trailingStop":true,"partialTake":true},"reverse":{"enabled":false},"risk":{"maxDailyLoss":15,"maxDrawdown":25},"fee":{"maker":0.0002,"taker":0.0004}}',
      '⚠️【高风险】低波动重仓等待大行情突破，波动阈值80 USDT。4级金字塔加仓，过滤假突破。',
      1, 304, NOW(), NOW()
    );
...
WHERE g.group_key = 'official_binance_btcusdt_v1'
GROUP BY g.id;
...
SELECT 
  ...
  leverage || 'x' AS "杠杆",
  margin_percent || '%' AS "仓位",
  stop_loss_percent || '%' AS "止损",
  volatility_threshold AS "波动阈值(USDT)"
FROM hg_trading_strategy_template
WHERE group_id = (SELECT id FROM hg_trading_strategy_group WHERE group_key = 'official_binance_btcusdt_v1')
ORDER BY sort;
