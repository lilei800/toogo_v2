        ...
        return h(
          ...
          { size: 'small' },
          {
            default: () => [
              h(
                NTag,
                { type: marketTag.type, size: 'small', bordered: false },
                { default: () => marketTag.text },
              ),
              h(
                NTag,
                { type: riskTag.type, size: 'small', bordered: false },
                { default: () => riskTag.text },
              ),
            ],
          },
        );
      ...
      render(row: any) {
        return h('div', { style: { fontSize: '12px', lineHeight: '1.6' } }, [
          ...
          h(
            'div',
            { style: { color: '#18a058' } },
            `窗口低: ${row.windowMinPrice?.toFixed(2) || '-'}`,
          ),
          h(
            'div',
            { style: { color: '#d03050' } },
            `窗口高: ${row.windowMaxPrice?.toFixed(2) || '-'}`,
          ),
          row.threshold
            ? h('div', { style: { color: '#666' } }, `阈值: ${row.threshold.toFixed(2)}`)
            : null,
        ]);
      },
    },
