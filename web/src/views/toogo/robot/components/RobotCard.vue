<template>
  ...
  >
    ...
    <template v-if="robot.status === 2">
      <slot name="running-content" :robot="robot" />
    </template>
        ...
        <n-grid :cols="4" :x-gap="8" :y-gap="8">
          ...
          <n-gi>
            <div class="param-box">
              ...
              <div class="param-value success">{{ robot.takeProfitRetracePercent || robot.profitRetreatPercent || '--' }}%</div>
            </div>
          </n-gi>
        </n-grid>
        ...
        <n-grid :cols="2" :x-gap="16">
          <n-gi>
            <div class="stat-box">
              ...
              <n-text :type="(robot.totalPnl || 0) >= 0 ? 'success' : 'error'" strong class="stat-value">
    ...
    <template #action>
      <n-space justify="space-between" style="width: 100%">
        <n-space align="center">
          <n-button v-if="robot.status === 2" type="warning" size="small" @click="$emit('stop', robot)">
            <template #icon><n-icon :component="PauseCircleOutlined" /></template>
            暂停
          </n-button>
          <n-button v-else-if="robot.status === 3 || robot.status === 1" type="primary" size="small" @click="$emit('start', robot)">
        ...
        <n-space>
          ...
          <n-button type="error" ghost size="small" @click="$emit('delete', robot)" :disabled="robot.status === 2">删除</n-button>
        </n-space>
      </n-space>
    </template>
  </n-card>
