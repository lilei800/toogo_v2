      ...
      <n-gi v-for="robot in robotList" :key="robot.id">
        ...
        >
          <template #running-content="{ robot: r }">
            ...
            <MarketAnalysisPanel 
              :robot="r" 
              :data="analysisData[r.id]" 
            />

            <!-- 信号预警面板 -->
            <SignalAlertPanel 
              :data="analysisData[r.id]" 
              :ticker="tickerData[r.id]" 
            />
              ...
              <n-grid :cols="2" :x-gap="12">
                ...
                <n-gi>
                  <div class="account-item">
                    ...
                    <n-text 
                      :type="(analysisData[r.id]?.account?.unrealizedPnl || 0) >= 0 ? 'success' : 'error'" 
                      strong 
