<template>
  <div class="toogo-admin-wallet">
    <n-card title="💰 用户钱包管理" :bordered="false">
      ...
      <n-form inline :show-feedback="false" style="margin-bottom: 16px">
        <n-form-item label="用户名">
          <n-input v-model:value="searchForm.username" placeholder="搜索用户名" clearable style="width: 150px" />
        </n-form-item>
        <n-form-item label="手机号">
          <n-input v-model:value="searchForm.mobile" placeholder="搜索手机号" clearable style="width: 150px" />
        </n-form-item>
        <n-form-item>
          <n-space>
    ...
    <!-- 充值弹窗 -->
    <n-modal v-model:show="showRechargeModal" preset="card" title="💰 手动充值" style="width: 520px">
      <n-alert type="info" style="margin-bottom: 16px">
        为用户 <strong>{{ rechargeForm.username }}</strong> (ID: {{ rechargeForm.userId }}) 进行手动充值
      </n-alert>
      ...
      <n-form ref="rechargeFormRef" :model="rechargeForm" :rules="rechargeRules" label-placement="left" label-width="100">
        <n-form-item label="当前余额">
          <n-space>
            <n-tag type="primary" size="large">💵 余额: {{ rechargeForm.currentBalance?.toFixed(2) || 0 }} U</n-tag>
            <n-tag type="success" size="large">⚡ 算力: {{ rechargeForm.currentPower?.toFixed(2) || 0 }}</n-tag>
            <n-tag type="warning" size="large">🎁 积分: {{ rechargeForm.currentGiftPower?.toFixed(2) || 0 }}</n-tag>
          </n-space>
        </n-form-item>
        ...
        <n-form-item label="快捷金额">
          <n-space>
            <n-button v-for="amt in [10, 50, 100, 500, 1000, 5000]" :key="amt" @click="rechargeForm.amount = amt" size="small" :type="rechargeForm.amount === amt ? 'primary' : 'default'">
              {{ amt }}
            </n-button>
          </n-space>
        </n-form-item>
        ...
        <n-form-item label="备注">
          <n-input v-model:value="rechargeForm.remark" type="textarea" :rows="2" placeholder="充值原因/备注（可选）" />
        </n-form-item>
      </n-form>
      ...
      <template #footer>
        <n-space justify="end">
          ...
          <n-button type="primary" :loading="rechargeLoading" @click="handleRecharge">
            <template #icon><n-icon><WalletOutlined /></n-icon></template>
            确认充值
          </n-button>
        </n-space>
      </template>
...
const columns = [
  ...
  { title: '手机号', key: 'mobile', width: 130 },
  {
    ...
    key: 'balance',
    width: 120,
    render: (row: any) => h('span', { style: 'color: #2080f0; font-weight: 600; font-family: monospace' }, (row.balance || 0).toFixed(2)),
  },
  {
    title: '算力',
    key: 'power',
    width: 120,
    render: (row: any) => h('span', { style: 'color: #18a058; font-weight: 600; font-family: monospace' }, (row.power || 0).toFixed(2)),
  },
  {
    title: '积分',
    key: 'giftPower',
    width: 100,
    render: (row: any) => h('span', { style: 'color: #f0a020; font-weight: 600; font-family: monospace' }, (row.giftPower || 0).toFixed(2)),
  },
  {
    title: '总可用算力',
    key: 'totalPower',
    width: 120,
    render: (row: any) => h(NTag, { type: 'success', size: 'small' }, () => (row.totalPower || 0).toFixed(2)),
  },
  {
    title: '佣金',
    key: 'commission',
    width: 100,
    render: (row: any) => h('span', { style: 'color: #8b5cf6; font-family: monospace' }, (row.commission || 0).toFixed(2)),
  },
  {
    title: 'VIP等级',
    key: 'vipLevel',
    width: 80,
    render: (row: any) => row.vipLevel > 0 
...
async function handleRecharge() {
  ...
  try {
    ...
    if (res.code === 0) {
      ...
      message.success(
        `充值成功！${typeMap[rechargeForm.value.accountType]}：${res.data?.beforeAmount?.toFixed(2)} → ${res.data?.afterAmount?.toFixed(2)}`
      );
