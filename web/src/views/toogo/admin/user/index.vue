  ...
  <div class="user-admin-page">
    ...
    <n-modal v-model:show="showDetailModal" title="用户详情" preset="card" style="width: 900px">
      <n-descriptions :column="2" label-placement="left" bordered v-if="currentRow">
        ...
        <n-descriptions-item label="订阅返佣"
          >{{ currentRow.subscribeRate || 0 }}%</n-descriptions-item
        >
        ...
        <n-descriptions-item label="消耗算力">{{
          currentRow.totalConsumePower
        }}</n-descriptions-item>
        <n-descriptions-item label="团队消耗">{{
          currentRow.teamConsumePower
        }}</n-descriptions-item>
        <n-descriptions-item label="当前套餐">{{
          currentRow.currentPlanId || '免费版'
        }}</n-descriptions-item>
        <n-descriptions-item label="套餐到期">{{
          currentRow.planExpireTime || '-'
        }}</n-descriptions-item>
        <n-descriptions-item label="机器人限制">{{ currentRow.robotLimit }}</n-descriptions-item>
        <n-descriptions-item label="运行中机器人">{{
          currentRow.activeRobotCount
        }}</n-descriptions-item>
        <n-descriptions-item label="算力折扣">{{ currentRow.powerDiscount }}%</n-descriptions-item>
        <n-descriptions-item label="代理商申请时间">{{
          currentRow.agentApplyAt || '-'
        }}</n-descriptions-item>
      ...
      <n-space>
        ...
        <n-button v-if="currentRow.agentStatus === 1" type="error" @click="openApproveModal(false)">
          拒绝申请
        </n-button>
    ...
    <n-modal
      ...
      :title="approveForm.approved ? '审批通过' : '拒绝申请'"
      preset="dialog"
      style="width: 500px"
    >
      ...
      <template #action>
        <n-space>
          ...
          <n-button
            :type="approveForm.approved ? 'success' : 'error'"
            @click="submitApprove"
    ...
    <!-- 充值算力弹窗 -->
    <n-modal
      v-model:show="showRechargePowerModal"
      title="手动充值算力"
      preset="dialog"
      style="width: 450px"
    >
      <n-form
        ref="rechargePowerFormRef"
        :model="rechargePowerForm"
        :rules="rechargePowerRules"
      ...
      <template #action>
        <n-space>
          ...
          <n-button type="primary" @click="submitRechargePower" :loading="rechargeLoading"
            >确认充值</n-button
          >
        </n-space>
      </template>
    ...
    <!-- 充值余额弹窗 -->
    <n-modal
      v-model:show="showRechargeBalanceModal"
      title="手动充值余额"
      preset="dialog"
      style="width: 450px"
    >
      <n-form
        ref="rechargeBalanceFormRef"
        :model="rechargeBalanceForm"
        :rules="rechargeRules"
        label-placement="left"
        label-width="80"
      >
      ...
      <template #action>
        <n-space>
          ...
          <n-button type="primary" @click="submitRechargeBalance" :loading="rechargeBalanceLoading"
            >确认充值</n-button
          >
        </n-space>
      </template>
    ...
    <!-- 充值积分弹窗 -->
    <n-modal
      v-model:show="showRechargePointsModal"
      title="手动充值积分"
      preset="dialog"
      style="width: 450px"
    >
      <n-form
        ref="rechargePointsFormRef"
        :model="rechargePointsForm"
        :rules="rechargeRules"
        label-placement="left"
        label-width="80"
      >
      ...
      <template #action>
        <n-space>
          ...
          <n-button type="primary" @click="submitRechargePoints" :loading="rechargePointsLoading"
            >确认充值</n-button
          >
        </n-space>
      </template>
  ...
  const columnsUsers = [
    ...
    },
    {
      ...
      render: (row: any) =>
        h(
          NTag,
          { type: row.vipLevel > 5 ? 'warning' : 'default', size: 'small' },
          { default: () => `V${row.vipLevel}` },
        ),
    },
    {
      title: '代理商',
    ...
    },
    {
      ...
      render: (row: any) =>
        h(
          NTag,
          { type: getAgentStatusType(row.agentStatus), size: 'small' },
          { default: () => getAgentStatusText(row.agentStatus) },
        ),
    },
    {
      title: '佣金比例',
    ...
    {
      title: '消耗算力',
      key: 'totalConsumePower',
      width: 80,
      render: (row: any) => (row.totalConsumePower || 0).toFixed(2),
    },
    {
      title: '机器人',
      key: 'robotLimit',
      width: 80,
      render: (row: any) => `${row.activeRobotCount}/${row.robotLimit}`,
    },
    {
      title: '状态',
      key: 'status',
  ...
  const columnsAgentApply = [
    ...
    },
    {
      ...
      render: (row: any) => {
        ...
        return h(
          NTag,
          { type: isAgent ? 'success' : 'default', size: 'small' },
          { default: () => (isAgent ? '代理' : '用户') },
        );
      },
    },
    {
  ...
  const submitRechargePower = async () => {
    ...
    try {
      const res = await ToogoUserApi.adminRechargePower({
        ...
        remark:
          rechargePowerForm.value.remark || `管理员手动充值 ${rechargePowerForm.value.amount} 算力`,
      });
      message.success(
        `充值成功！用户 ${currentRow.value.username} 新的算力余额为: ${
          res?.newBalance || rechargePowerForm.value.amount
        }`,
      );
  ...
  const submitRechargeBalance = async () => {
    ...
    try {
      const res = await ToogoUserApi.adminRechargeBalance({
        ...
        remark:
          rechargeBalanceForm.value.remark ||
          `管理员手动充值 ${rechargeBalanceForm.value.amount} USDT`,
      });
      // 更新 currentRow 的余额
      ...
      message.success(
        `充值成功！用户 ${currentRow.value.username} 新的余额为: ${
          res?.newBalance || rechargeBalanceForm.value.amount
        } USDT`,
      );
  ...
  const submitRechargePoints = async () => {
    ...
    try {
      const res = await ToogoUserApi.adminRechargePoints({
        ...
        remark:
          rechargePointsForm.value.remark ||
          `管理员手动充值 ${rechargePointsForm.value.amount} 积分`,
      });
      // 更新 currentRow 的积分
      if (currentRow.value && res?.newBalance !== undefined) {
      ...
      message.success(
        `充值成功！用户 ${currentRow.value.username} 新的积分余额为: ${
          res?.newBalance || rechargePointsForm.value.amount
        }`,
      );
