        ...
        <n-grid :cols="2" :x-gap="16">
          ...
          <n-gi>
            <n-form-item label="机器人限制" path="robotLimit">
              <n-input-number
                v-model:value="editForm.robotLimit"
                :min="1"
                :max="10000"
                style="width: 100%"
              />
          ...
          <n-gi>
            <n-form-item label="是否限制购买">
              <n-switch
                v-model:value="editForm.purchaseLimitEnabled"
                :checked-value="1"
                :unchecked-value="0"
              />
            </n-form-item>
          ...
          <n-gi>
            <n-form-item label="月付赠送">
              <n-input-number
                v-model:value="editForm.giftPowerMonthly"
                :min="0"
                style="width: 100%"
              />
            </n-form-item>
          ...
          <n-gi>
            <n-form-item label="季付赠送">
              <n-input-number
                v-model:value="editForm.giftPowerQuarterly"
                :min="0"
                style="width: 100%"
              />
            </n-form-item>
          ...
          <n-gi>
            <n-form-item label="半年赠送">
              <n-input-number
                v-model:value="editForm.giftPowerHalfYear"
                :min="0"
                style="width: 100%"
              />
            </n-form-item>
          ...
          <n-gi>
            <n-form-item label="年付赠送">
              <n-input-number
                v-model:value="editForm.giftPowerYearly"
                :min="0"
                style="width: 100%"
              />
            </n-form-item>
          ...
          <n-gi>
            <n-form-item label="推荐套餐">
              <n-switch
                v-model:value="editForm.isDefault"
                :checked-value="1"
                :unchecked-value="0"
              />
            </n-form-item>
...
<script setup lang="ts">
  ...
  const handlePeriodChange = (index: number, newPeriodType: string) => {
    // 检查是否已存在该周期
    const existingIndex = priceList.value.findIndex(
      (item, idx) => idx !== index && item.periodType === newPeriodType,
    );
    if (existingIndex !== -1) {
  ...
  const syncPriceListToForm = () => {
    ...
    if (priceList.value.length > 0) {
      const hasDefaultPeriod = priceList.value.some(
        (item) => item.periodType === editForm.value.defaultPeriod,
      );
      if (!hasDefaultPeriod) {
  ...
  const syncFormToPriceList = () => {
    ...
    // 添加所有价格 > 0 的方案
    if (editForm.value.priceDaily > 0)
      prices.push({ periodType: 'daily', price: editForm.value.priceDaily || 0 });
    if (editForm.value.priceMonthly > 0)
      prices.push({ periodType: 'monthly', price: editForm.value.priceMonthly || 0 });
    if (editForm.value.priceQuarterly > 0)
      prices.push({ periodType: 'quarterly', price: editForm.value.priceQuarterly || 0 });
    if (editForm.value.priceHalfYear > 0)
      prices.push({ periodType: 'half_year', price: editForm.value.priceHalfYear || 0 });
    if (editForm.value.priceYearly > 0)
      prices.push({ periodType: 'yearly', price: editForm.value.priceYearly || 0 });
  ...
  const columns = [
    ...
    { title: '机器人限制', key: 'robotLimit' },
    {
      ...
      render: (row: any) =>
        h(
          NTag,
          { type: Number(row.purchaseLimit) > 0 ? 'warning' : 'default', size: 'small' },
          { default: () => (Number(row.purchaseLimit) > 0 ? '限制' : '不限') },
        ),
    },
    {
      title: '限购次数',
  ...
  const loadData = async () => {
    loading.value = true;
    try {
      const res = await ToogoSubscriptionApi.planList({
  ...
  const handleSave = async () => {
    ...
    // 验证推荐周期是否在价格列表中
    const hasRecommendedPeriod = priceList.value.some(
      (item) => item.periodType === editForm.value.defaultPeriod,
    );
