  ...
  <div class="global-engine-page">
    ...
    <n-grid :cols="5" :x-gap="16" class="stat-cards">
      ...
      <n-gi>
        <n-card class="stat-card">
          <n-statistic
            label="活跃机器人"
            :value="engineDetail?.robotTaskManager?.activeRobots || 0"
          >
            <template #suffix>
      ...
      <n-gi>
        <n-card class="stat-card">
          <n-statistic
            label="行情订阅"
            :value="engineDetail?.marketDataService?.subscriptions || 0"
          >
            <template #suffix>
    ...
    <n-card title="行情数据服务" :bordered="false" class="proCard" style="margin-top: 16px">
      <template #header-extra>
        <n-space>
          <n-tag
            :type="engineDetail?.marketDataService?.running ? 'success' : 'error'"
            size="small"
          >
    ...
    <n-grid :cols="2" :x-gap="16" style="margin-top: 16px">
        ...
        <n-card title="预警日志服务" :bordered="false" class="proCard">
          ...
          <n-grid :cols="3" :x-gap="16">
            <n-gi>
              <n-statistic
                label="市场状态日志"
                :value="engineDetail?.alertLogger?.marketStateLogs || 0"
              />
            </n-gi>
            <n-gi>
              <n-statistic
                label="风险偏好日志"
                :value="engineDetail?.alertLogger?.riskPreferenceLogs || 0"
              />
            </n-gi>
            <n-gi>
              <n-statistic
                label="方向日志"
                :value="engineDetail?.alertLogger?.directionLogs || 0"
              />
            </n-gi>
          </n-grid>
        </n-card>
      ...
      <n-gi>
        <n-card title="交易统计服务" :bordered="false" class="proCard">
          <template #header-extra>
            <n-tag
              :type="engineDetail?.tradeStatistics?.running ? 'success' : 'error'"
              size="small"
            >
          ...
          <n-grid :cols="4" :x-gap="16">
            <n-gi>
              <n-statistic
                label="总交易数"
                :value="engineDetail?.tradeStatistics?.totalTrades || 0"
              />
            </n-gi>
            <n-gi>
              <n-statistic
                label="今日交易"
                :value="engineDetail?.tradeStatistics?.todayTrades || 0"
              />
            </n-gi>
            <n-gi>
              <n-statistic label="总盈亏 (USDT)">
                <span
                  :style="{
                    color:
                      (engineDetail?.tradeStatistics?.totalProfit || 0) >= 0
                        ? '#18a058'
                        : '#d03050',
                  }"
                >
            ...
            <n-gi>
              <n-statistic label="今日盈亏 (USDT)">
                <span
                  :style="{
                    color:
                      (engineDetail?.tradeStatistics?.todayProfit || 0) >= 0
                        ? '#18a058'
                        : '#d03050',
                  }"
                >
...
<script lang="ts" setup>
  ...
  import { getGlobalEngineDetail, restartGlobalEngine } from '@/api/trading/alert';
  ...
  const tickerColumns = [
    ...
    },
    {
      ...
      width: 100,
      render: (row: any) =>
        h(NTag, { size: 'small', type: 'info' }, () => row.platform?.toUpperCase()),
    },
    {
      title: '最新价',
      key: 'lastPrice',
  ...
  const analysisColumns = [
    ...
    },
    {
      ...
      render: (row: any) => {
        ...
        const state = stateMap[row.marketState] || {
          text: row.marketState || '-',
          type: 'default',
        };
