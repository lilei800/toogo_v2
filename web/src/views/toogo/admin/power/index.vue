      ...
      <n-tab-pane name="wallet" tab="💰 用户钱包管理">
        <n-card :bordered="false">
          ...
          <n-space style="margin-bottom: 16px">
            <n-input
              v-model:value="walletSearch.username"
              placeholder="用户名搜索"
              clearable
              style="width: 160px"
            />
            <n-input
              v-model:value="walletSearch.mobile"
              placeholder="手机号搜索"
              clearable
              style="width: 160px"
            />
      ...
      <n-tab-pane name="config" tab="⚙️ 算力配置">
        <n-card :bordered="false">
          ...
          >
            ...
            <n-form-item
              v-for="(level, index) in config.vipDiscounts"
              :key="index"
              :label="`V${index + 1} 优惠比例`"
            >
              <n-input-number
...
<script lang="ts" setup>
  ...
  const walletColumns = [
    ...
    { title: '手机号', key: 'mobile', width: 130 },
    {
      ...
      width: 110,
      render: (row: any) =>
        h('span', { style: 'color: #2080f0; font-weight: 600' }, row.balance?.toFixed(2) || '0.00'),
    },
    {
      title: '算力',
      key: 'power',
      width: 100,
      render: (row: any) =>
        h('span', { style: 'color: #18a058; font-weight: 600' }, row.power?.toFixed(2) || '0.00'),
    },
    {
      title: '积分',
      key: 'giftPower',
      ...
      render: (row: any) =>
        h(
          'span',
          { style: 'color: #f0a020; font-weight: 600' },
          row.giftPower?.toFixed(2) || '0.00',
        ),
    },
    {
      title: '总算力',
      ...
      width: 100,
      render: (row: any) =>
        h(NTag, { type: 'success', size: 'small' }, () => row.totalPower?.toFixed(2) || '0.00'),
    },
    {
      title: 'VIP',
      key: 'vipLevel',
    ...
    },
    {
      ...
      render: (row: any) =>
        h(NSpace, { size: 'small' }, () => [
          h(
            NButton,
            { type: 'primary', size: 'small', onClick: () => openRechargeModal(row) },
            () => '充值',
          ),
        ]),
    },
