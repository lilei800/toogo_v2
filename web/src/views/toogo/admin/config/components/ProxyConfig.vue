      ...
      >
        ...
        <n-form-item label="代理地址" path="proxyAddress">
          ...
          <span class="form-desc"
            >格式：IP:端口，HTTP示例：127.0.0.1:33210（已验证可用），SOCKS5示例：127.0.0.1:33211</span
          >
        </n-form-item>
      ...
      >
        <template v-if="testResult.success && testResult.externalIp">
          <n-descriptions label-placement="left" bordered :column="1" style="margin-top: 20px">
            ...
            <n-descriptions-item label="延迟"> {{ testResult.latency }}ms </n-descriptions-item>
          </n-descriptions>
        </template>
      </n-result>
    </n-card>
    ...
    <n-card title="配置说明" :bordered="false" style="margin-top: 16px">
      <n-alert type="info" :bordered="false">
        <n-ul style="margin: 0; padding-left: 20px">
          ...
          <li
            ><strong>使用场景</strong
            >：代理配置仅用于API对接交易所平台（如Bitget、Binance、OKX等）</li
          >
          <li
            ><strong>适用场景</strong>：用户API配置测试、交易机器人执行、订单查询等需要API
            Key的操作</li
          >
          ...
          <li
            ><strong>注意</strong
            >：如果SOCKS5代理测试失败，提示"可能不是SOCKS5代理"，说明该端口可能是HTTP代理，请改用HTTP代理类型</li
          >
          <li
            >代理地址格式：IP:端口，HTTP示例：127.0.0.1:33210（已验证可用），SOCKS5示例：127.0.0.1:33211（如果测试失败，请检查代理类型）</li
          >
...
<script lang="ts" setup>
  ...
  const loadConfig = async () => {
    try {
      ...
      if (configData) {
        ...
        formData.proxyAddress = configData.proxyAddress || '127.0.0.1:10808';
  ...
  const handleTest = async () => {
    try {
      ...
      // http.request 默认 isTransformResponse=true，成功时直接返回 data 字段（即 TradingProxyConfigTestModel）
      testResult.value = {
        success: res?.success,
        message: res?.message,
