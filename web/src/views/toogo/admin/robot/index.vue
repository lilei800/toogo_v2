      ...
      <n-drawer-content :title="`机器人详情 - ${detailRobot?.name}`">
        <template v-if="detailRobot">
          ...
          <n-descriptions label-placement="left" bordered :column="2">
            ...
            <n-descriptions-item label="保证金模式">{{
              detailRobot.marginMode
            }}</n-descriptions-item>
            <n-descriptions-item label="最大盈利"
              >{{ detailRobot.maxProfit }} USDT</n-descriptions-item
            >
            <n-descriptions-item label="最大亏损"
              >{{ detailRobot.maxLoss }} USDT</n-descriptions-item
            >
            <n-descriptions-item label="止损比例">{{ detailRobot.stopLoss }}%</n-descriptions-item>
            <n-descriptions-item label="止盈回撤"
              >{{ detailRobot.trailingStop }}%</n-descriptions-item
            >
          </n-descriptions>
          ...
          <n-descriptions label-placement="left" bordered :column="2">
            <n-descriptions-item label="累计盈亏">
              <span :style="{ color: detailRobot.totalPnl >= 0 ? '#18a058' : '#d03050' }">
                {{ detailRobot.totalPnl >= 0 ? '+' : ''
                }}{{ formatNumber(detailRobot.totalPnl) }} USDT
              </span>
            </n-descriptions-item>
            <n-descriptions-item label="消耗算力">{{
              formatNumber(detailRobot.powerUsed)
            }}</n-descriptions-item>
        ...
        <template #footer>
          <n-space>
            <n-popconfirm
              @positive-click="handleForceStop(detailRobot)"
              v-if="detailRobot?.status === 'running'"
            >
              <template #trigger>
...
<script lang="ts" setup>
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: any) => {
        ...
        return h(
          'span',
          { style: { color: pnl >= 0 ? '#18a058' : '#d03050', fontWeight: 'bold' } },
  ...
  const orderColumns = [
    { title: '时间', key: 'time', width: 150 },
    {
      ...
      width: 60,
      render: (row: any) =>
        h(NTag, { type: row.side === 'long' ? 'success' : 'error', size: 'small' }, () =>
          row.side === 'long' ? '做多' : '做空',
        ),
    },
    ...
    { title: '数量', key: 'amount', width: 80 },
    {
      ...
      render: (row: any) =>
        h(
          'span',
          { style: { color: row.pnl >= 0 ? '#18a058' : '#d03050' } },
