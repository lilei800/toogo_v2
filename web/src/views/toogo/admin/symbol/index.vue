    ...
    >
      <n-form :model="addForm" label-placement="left" label-width="80">
        ...
        <n-form-item label="价格精度">
          <n-input-number v-model:value="addForm.pricePrecision" :min="0" :max="10" style="width: 100%" />
        </n-form-item>
        <n-form-item label="数量精度">
          <n-input-number v-model:value="addForm.qtyPrecision" :min="0" :max="10" style="width: 100%" />
        </n-form-item>
      </n-form>
    </n-modal>
    ...
    >
      <n-form v-if="editingSymbol" :model="editingSymbol" label-placement="left" label-width="80">
        ...
        <n-form-item label="最大杠杆">
          <n-input-number v-model:value="editingSymbol.maxLeverage" :min="1" :max="200" style="width: 100%" />
        </n-form-item>
...
import { useMessage, useDialog, NButton, NTag, NSpace, NIcon, NSwitch, NPopconfirm } from 'naive-ui';
import { SyncOutlined, PlusOutlined, SearchOutlined, EditOutlined, DeleteOutlined } from '@vicons/antd';
...
const filteredSymbols = computed(() => {
  return symbols.value.filter((s) => {
    const matchKey = !searchKey.value || s.symbol.toLowerCase().includes(searchKey.value.toLowerCase());
...
const columns = [
  ...
  { title: '最大杠杆', key: 'maxLeverage', width: 80 },
  {
    ...
    key: 'isHot',
    width: 60,
    render: (row: any) => row.isHot ? h(NTag, { type: 'warning', size: 'small' }, () => '热门') : '-',
  },
  {
    title: '状态',
    key: 'status',
