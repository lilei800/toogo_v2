  ...
  <div class="withdraw-admin-page">
    ...
    <n-modal v-model:show="showAuditModal" title="提现审核" preset="card" style="width: 500px">
      <n-descriptions :column="1" label-placement="left" bordered v-if="currentRow">
        ...
        <n-descriptions-item label="提现账户">{{
          currentRow.accountType === 'balance' ? '余额' : '佣金'
        }}</n-descriptions-item>
        ...
        <n-descriptions-item label="实际到账"
          >{{ currentRow.actualAmount }} USDT</n-descriptions-item
        >
      ...
      <n-form :model="auditForm" label-placement="left" label-width="80">
        ...
        <n-form-item label="备注">
          <n-input
            v-model:value="auditForm.auditNote"
            type="textarea"
            :rows="2"
          ...
          {},
          {
            default: () => [
              row.status === 1
                ? h(
                    NButton,
                    { size: 'small', type: 'primary', onClick: () => openAuditModal(row) },
                    { default: () => '审核' },
                  )
                : h(
                    NButton,
