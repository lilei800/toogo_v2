<template>
  <div class="agent-level-admin-page">
    ...
    <!-- 编辑弹窗 -->
    <n-modal v-model:show="showEditModal" :title="editForm.id ? '编辑代理商等级' : '新增代理商等级'" preset="card" style="width: 700px">
      <n-form ref="formRef" :model="editForm" :rules="rules" label-placement="left" label-width="140">
        <n-grid :cols="2" :x-gap="16">
          <n-gi>
            <n-form-item label="等级" path="level">
              <n-input-number v-model:value="editForm.level" :min="1" :max="10" style="width: 100%" />
            </n-form-item>
          </n-gi>
          <n-gi>
          ...
          <n-gi>
            <n-form-item label="团队人数要求">
              <n-input-number v-model:value="editForm.requireTeamCount" :min="0" style="width: 100%" />
            </n-form-item>
          </n-gi>
          <n-gi>
            <n-form-item label="团队订阅金额">
              <n-input-number v-model:value="editForm.requireTeamSubscribe" :min="0" :precision="2" style="width: 100%" />
            </n-form-item>
          </n-gi>
          <n-gi :span="2">
          ...
          <n-gi>
            <n-form-item label="一级订阅佣金">
              <n-input-number v-model:value="editForm.subscribeRate1" :min="0" :max="1" :precision="2" :step="0.01" style="width: 100%">
          ...
          <n-gi>
            <n-form-item label="二级订阅佣金">
              <n-input-number v-model:value="editForm.subscribeRate2" :min="0" :max="1" :precision="2" :step="0.01" style="width: 100%">
          ...
          <n-gi>
            <n-form-item label="三级订阅佣金">
              <n-input-number v-model:value="editForm.subscribeRate3" :min="0" :max="1" :precision="2" :step="0.01" style="width: 100%">
          ...
          <n-gi>
            <n-form-item label="一级算力佣金">
              <n-input-number v-model:value="editForm.powerRate1" :min="0" :max="1" :precision="2" :step="0.01" style="width: 100%">
          ...
          <n-gi>
            <n-form-item label="二级算力佣金">
              <n-input-number v-model:value="editForm.powerRate2" :min="0" :max="1" :precision="2" :step="0.01" style="width: 100%">
          ...
          <n-gi>
            <n-form-item label="三级算力佣金">
              <n-input-number v-model:value="editForm.powerRate3" :min="0" :max="1" :precision="2" :step="0.01" style="width: 100%">
...
const columns = [
  { title: '等级', key: 'level', render: (row: any) => h(NTag, { type: 'info' }, { default: () => `Lv.${row.level}` }) },
  { title: '等级名称', key: 'levelName' },
  { title: '团队人数', key: 'requireTeamCount' },
  { title: '团队订阅', key: 'requireTeamSubscribe' },
  { title: '一级订阅', key: 'subscribeRate1', render: (row: any) => `${(row.subscribeRate1 * 100).toFixed(0)}%` },
  { title: '二级订阅', key: 'subscribeRate2', render: (row: any) => `${(row.subscribeRate2 * 100).toFixed(0)}%` },
  { title: '三级订阅', key: 'subscribeRate3', render: (row: any) => `${(row.subscribeRate3 * 100).toFixed(0)}%` },
  { title: '一级算力', key: 'powerRate1', render: (row: any) => `${(row.powerRate1 * 100).toFixed(0)}%` },
  {
    title: '状态',
    key: 'status',
  ...
  },
  {
    ...
    key: 'actions',
    width: 100,
    render: (row: any) => h(NButton, { size: 'small', onClick: () => openEditModal(row) }, { default: () => '编辑' }),
  },
];
