    ...
    >
      ...
      >
        ...
        <n-form-item label="API Key" path="apiKey">
          <n-input v-model:value="formData.apiKey" placeholder="请输入API Key" />
        </n-form-item>
        ...
        <n-form-item v-if="needPassphrase" label="Passphrase" path="passphrase">
          <n-input
            ...
            type="password"
            placeholder="请输入Passphrase（OKX/Bitget必填）"
            show-password-on="click"
          />
        </n-form-item>
    ...
    <n-drawer v-model:show="showGuideDrawer" :width="720" placement="right">
      <n-drawer-content title="交易所API开通教程" closable>
        <n-tabs type="line" animated>
          ...
          <n-tab-pane name="binance" tab="Binance (币安)">
            <div class="guide-content">
              <n-alert type="warning" style="margin-bottom: 16px;">
                ...
                <strong>重要提示：</strong>请务必启用"现货交易"权限，禁用"提现"权限，并绑定IP白名单以提升安全性。
              </n-alert>
                ...
                <n-step title="保存密钥信息">
                  <div class="step-content">
                    ...
                    <n-alert type="error" style="margin-top: 8px;">
                      <template #icon><n-icon><LockOutlined /></n-icon></template>
                      <strong>安全提示：</strong>Secret Key只会显示一次，请妥善保管，切勿泄露给他人！
                    </n-alert>
                  </div>
                </n-step>
              ...
              <n-steps vertical :current="1">
                ...
                <n-step title="创建API密钥">
                  <div class="step-content">
                    ...
                    <n-alert type="info" size="small" style="margin-top: 8px;">
                      <strong>注意：</strong>Passphrase是您自己设置的密码，请务必记住，丢失后无法找回！
                    </n-alert>
                  </div>
                </n-step>
          ...
          <!-- Bitget教程 -->
          <n-tab-pane name="bitget" tab="Bitget">
            <div class="guide-content">
              <n-alert type="warning" style="margin-bottom: 16px;">
                <template #icon>
                  <n-icon><WarningOutlined /></n-icon>
                </template>
                <strong>重要提示：</strong>Bitget同样需要设置Passphrase（API口令），请务必记住！
              </n-alert>

              <n-steps vertical :current="1">
                <n-step title="登录Bitget账户">
                  <div class="step-content">
                    <p>1. 访问 <n-text code>https://www.bitget.com</n-text> 并登录您的账户</p>
                    <p>2. 点击右上角头像，选择「API管理」</p>
                  </div>
                </n-step>
                ...
                <n-step title="创建API密钥">
                  <div class="step-content">
                    <p>1. 点击「创建API」按钮</p>
                    <p>2. 设置API备注（如：ToogoAI）</p>
                    <p>3. 设置API口令（Passphrase，至少6位）</p>
                    <p>4. 完成安全验证</p>
                  </div>
                </n-step>
                ...
                <n-step title="配置API权限">
                  <div class="step-content">
                    <n-space vertical>
                      <n-alert type="success" size="small">
                        <template #icon><n-icon><CheckCircleOutlined /></n-icon></template>
                        <strong>必须启用：</strong>现货交易权限
                      </n-alert>
                      <n-alert type="error" size="small">
                        <template #icon><n-icon><CloseCircleOutlined /></n-icon></template>
                        <strong>禁止启用：</strong>提现权限、划转权限
                      </n-alert>
                      <p style="margin-top: 8px;">其他权限可按需选择</p>
                    </n-space>
                  </div>
                </n-step>
                ...
                <n-step title="保存密钥信息">
                  <div class="step-content">
                    ...
                    <p>3. <n-text strong>Passphrase</n-text>（您设置的API口令）</p>
                  </div>
                </n-step>
        ...
        <n-card title="安全注意事项" size="small" :bordered="false" style="background: #f8f9fa;">
          <n-space vertical>
            <n-alert type="warning" size="small">
              <template #icon><n-icon><SafetyOutlined /></n-icon></template>
...
const needPassphrase = computed(() => {
  return ['okx', 'bitget'].includes(formData.value.exchange);
});
...
const columns = [
  ...
  { title: 'API名称', key: 'apiName', width: 150 },
  {
    ...
    render: (row: any) => {
      ...
      return h(NTag, { color: { color: colors[row.platform] || '#666', textColor: '#fff' } }, () => labels[row.platform] || row.platform);
    },
  },
  {
    title: 'API Key',
