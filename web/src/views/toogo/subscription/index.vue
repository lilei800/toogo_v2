<template>
  <div class="subscription-page">
      ...
      <n-gi>
        <n-card v-if="currentSubscription.hasSubscription" :bordered="true" size="small">
          <n-statistic label="当前订阅" :value="currentSubscription.planName">
            <template #suffix>
              <n-tag type="success" size="small" style="margin-left: 8px">剩余 {{ currentSubscription.remainingDays }} 天</n-tag>
            </template>
          </n-statistic>
        </n-card>
    ...
    <n-card title="订阅套餐" :bordered="true" size="small">
      ...
      <n-grid cols="1 s:2 m:2 l:3 xl:4 2xl:4" :x-gap="12" :y-gap="12" responsive="screen">
        <n-gi v-for="plan in planList" :key="plan.id">
          <n-card
            class="plan-card"
            :class="{ 'plan-popular': plan.isDefault === 1, 'plan-selected': selectedPlan?.id === plan.id }"
            hoverable
            size="small"
            :bordered="false"
          ...
          >
            ...
            <div class="plan-period-select" @click.stop>
              <n-text depth="3" style="font-size: 12px; display: block; margin-bottom: 6px">选择订阅周期</n-text>
              <n-select 
            ...
            <div class="plan-price-display">
              ...
              <div class="daily-price-hint">
                <n-text depth="3" style="font-size: 11px">
                  约 {{ getDailyPricePerRobot(plan, getPlanSelectedPeriod(plan.id)).toFixed(4) }} USDT/天/机器人
                </n-text>
              </div>
            </div>
            ...
            <div class="plan-action" @click.stop>
              <n-space vertical :size="6">
                <n-tag v-if="selectedPlan?.id === plan.id" type="success" size="medium" style="width: 100%; justify-content: center;">
                  ✓ 已选择
                </n-tag>
                <n-button 
                  text 
    ...
    <n-card v-if="selectedPlan" class="mt-3 order-panel" :bordered="true" size="small">
      ...
      <n-space vertical :size="16">
        ...
        <n-descriptions :column="2" label-placement="left" size="small" bordered>
          ...
          <n-descriptions-item label="订阅时长">
            {{ getPeriodDays() }} 天
          </n-descriptions-item>
          <n-descriptions-item label="赠送算力">
            {{ getGiftPower() }} 算力
          </n-descriptions-item>
        </n-descriptions>
        ...
        <n-card size="small" :bordered="true" title="支付方式">
          <n-radio-group v-model:value="payType" size="small">
            <n-space vertical :size="12">
              ...
              <n-alert v-if="!canUseBalance && payType === 'balance'" type="warning" size="small" :show-icon="true">
                余额不足，请先充值或使用其他支付方式
                <template #action>
        ...
        <n-card size="small" :bordered="true" title="费用明细" class="payment-summary">
          <n-space vertical :size="8">
            ...
            <n-space v-if="usePoints" align="center" justify="space-between">
              ...
              <n-text type="success">-{{ Math.min(userPoints, getSelectedPrice()).toFixed(2) }} 积分</n-text>
            </n-space>
            ...
            <n-alert v-if="payType === 'balance' && accountBalance < getFinalAmount()" type="error" size="small" :show-icon="true">
              余额不足，当前余额 {{ accountBalance.toFixed(2) }} USDT，需要 {{ getFinalAmount().toFixed(2) }} USDT
              <template #action>
                <n-button text type="primary" size="small" @click="$router.push('/toogo/finance')">
                  立即充值
                </n-button>
    ...
    <!-- 订阅记录 -->
    <n-card title="订阅记录" :bordered="false" :segmented="{ content: true }" size="small" class="mt-3">
      <n-data-table
...
const subscriptionColumns = [
  ...
  { 
    ...
    key: 'amount', 
    width: 100,
    render: (row: any) => h('span', { style: { color: 'var(--warning-color)', fontWeight: '600' } }, `${row.amount} USDT`)
  },
  ...
  dialog.info({
    ...
    content: () => h('div', { style: 'padding: 16px 0;' }, [
      h('div', { style: 'margin-bottom: 16px;' }, [
        h('n-text', { strong: true, style: 'font-size: 16px;' }, `支持 ${plan.robotLimit} 个云机器人同时运行`),
      ]),
      h('n-divider'),
      h('div', { style: 'margin-bottom: 12px;' }, [
        h('n-text', { strong: true }, '价格方案：'),
      ]),
      h('div', { style: 'margin-left: 16px;' }, pricingPeriods.map((period: any) => {
        ...
        return h('div', { 
          ...
          h('n-space', { align: 'center', size: 8 }, [
            ...
            period.discount ? h(NTag, { size: 'small', type: period.key === 'yearly' ? 'warning' : 'success' }, { default: () => period.discount }) : null,
          ]),
        ]);
      })),
      ...
      h('div', { style: 'margin-left: 16px;' }, [
        h('div', { class: 'feature-item', style: 'display: flex; align-items: center; gap: 8px; padding: 6px 0;' }, [
...
const loadData = async () => {
  ...
  try {
    const [planRes, subRes, mySubRes, walletRes] = await Promise.all([
      ...
      ToogoSubscriptionApi.subscriptionList({ page: pagination.value.page, perPage: pagination.value.pageSize }),
...
const doSubscribe = async () => {
  ...
  if (payType.value === 'balance') {
    ...
    if (accountBalance.value < finalAmount) {
      message.error(`余额不足，当前余额 ${accountBalance.value.toFixed(2)} USDT，需要 ${finalAmount.toFixed(2)} USDT`);
      return;
    }
  }
  ...
  dialog.warning({
    ...
    content: () =>
      h('div', { style: 'line-height: 1.8;' }, [
        ...
        h('div', [h('b', '支付方式：'), payType.value === 'balance' ? '余额支付' : payType.value]),
        usePoints.value ? h('div', [h('b', '积分抵扣：'), `${Math.min(userPoints.value, getSelectedPrice()).toFixed(2)} 积分`]) : null,
