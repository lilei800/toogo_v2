<template>
  <div class="team-page">
    ...
    <n-card :bordered="false" class="proCard mt-3" title="代理申请">
      <template #header-extra>
        <n-space align="center" :size="8">
          ...
          <n-tag
            v-if="agentInfo?.agentStatus === 2"
            :type="agentInfo?.agentUnlockLevel === 1 ? 'success' : 'default'"
            size="small"
          >
    ...
    <!-- 我申请代理 -->
    <n-modal
      v-model:show="showSelfApplyModal"
      preset="card"
      style="width: 560px"
      title="我要申请代理"
    >
      <n-space vertical :size="12">
        <n-form
          ...
          :model="selfApplyForm"
          :rules="selfApplyRules"
          label-width="130"
        >
          <n-form-item label="申请订阅返佣(%)" path="subscribeRate">
            <n-input-number
              v-model:value="selfApplyForm.subscribeRate"
              :min="0.01"
              :max="100"
              :precision="2"
              style="width: 100%"
          ...
          <n-form-item label="申请理由" path="remark">
            <n-input
              v-model:value="selfApplyForm.remark"
              type="textarea"
              :rows="3"
              placeholder="请输入申请理由"
            />
        ...
        <n-space justify="end">
          ...
          <n-button type="primary" :loading="selfApplyLoading" @click="submitSelfApply"
            >提交申请</n-button
          >
        </n-space>
      </n-space>
    ...
    <!-- 帮下级申请代理 -->
    <n-modal
      v-model:show="showSubApplyModal"
      preset="card"
      style="width: 560px"
      title="帮成员申请代理"
    >
      <n-space vertical :size="12">
        <n-text depth="3"
          >申请订阅返佣比例不能超过您的比例：{{ agentInfo?.subscribeRate ?? 0 }}%</n-text
        >
        <n-form
          ref="subApplyFormRef"
          :model="subApplyForm"
        ...
        >
          ...
          <n-form-item label="申请理由" path="remark">
            <n-input
              v-model:value="subApplyForm.remark"
              type="textarea"
              :rows="3"
              placeholder="请输入申请理由"
            />
        ...
        <n-space justify="end">
          ...
          <n-button type="primary" :loading="subApplyLoading" @click="submitSubApply"
            >提交申请</n-button
          >
        </n-space>
      </n-space>
    ...
    <!-- 设置下级代理佣金比例 -->
    <n-modal
      v-model:show="showSetRateModal"
      preset="card"
      style="width: 520px"
      title="设置下级代理佣金比例"
    >
      <n-space vertical :size="12">
        <n-text depth="3"> 上级可设置范围：订阅 ≤ {{ agentInfo?.subscribeRate ?? 0 }}% </n-text>
    ...
    <n-card title="佣金收入" :bordered="false" class="proCard mt-3">
      <n-grid cols="1 s:2 m:2 l:4 xl:4 2xl:4" :x-gap="12" :y-gap="12" responsive="screen">
        <n-gi>
          <n-statistic
            label="今日佣金"
            :value="commissionStat.todayCommission?.toFixed(4) || '0.0000'"
            suffix="USDT"
          />
        </n-gi>
        <n-gi>
          <n-statistic
            label="本周佣金"
            :value="commissionStat.weekCommission?.toFixed(4) || '0.0000'"
            suffix="USDT"
          />
        </n-gi>
        <n-gi>
          <n-statistic
            label="本月佣金"
            :value="commissionStat.monthCommission?.toFixed(4) || '0.0000'"
            suffix="USDT"
          />
        </n-gi>
        <n-gi>
          <n-statistic
            label="累计佣金"
            :value="commissionStat.totalCommission?.toFixed(4) || '0.0000'"
            suffix="USDT"
          />
        </n-gi>
      </n-grid>
...
<script setup lang="ts">
  ...
  const filteredTeamList = computed(() => {
    ...
    return (teamList.value || []).filter((x: any) =>
      String(x?.username || '')
        .toLowerCase()
        .includes(k),
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: any) => {
        return h(
          NTag,
          { type: row.vipLevel > 1 ? 'warning' : 'default', size: 'small' },
  ...
  const openSubApplyModal = (row: any) => {
    ...
    subApplyForm.value.subscribeRate =
      Math.min(Number(agentInfo.value?.subscribeRate ?? 0), 10) || 10;
