<template>
  <div class="strategy-list">
    <n-card :bordered="false" class="proCard">
      ...
      <!-- 策略矩阵视图 -->
      <n-alert
        v-if="strategies.length < 12 && !isReadonly"
        type="warning"
        style="margin-bottom: 16px"
      >
        当前仅有 {{ strategies.length }}/12
        种策略，建议点击"批量初始化"补全所有市场状态和风险偏好组合。
      </n-alert>
      ...
      <n-grid :cols="3" :x-gap="16" :y-gap="16">
            ...
            <n-space vertical :size="8">
              <n-space>
                <n-tag size="small" :bordered="false">{{
                  getMarketLabel(strategy.marketState)
                }}</n-tag>
              </n-space>
        ...
        <n-gi v-if="filteredStrategies.length === 0" :span="3">
          <n-empty
            :description="`暂无${activeMarket === 'all' ? '' : getMarketLabel(activeMarket)}策略`"
          >
            ...
            <n-grid :cols="2" :x-gap="16">
              ...
              <n-gi>
                <n-form-item label="保证金比例" path="marginPercent">
                  <n-input-number
                    v-model:value="formData.marginPercent"
                    :min="1"
                    :max="100"
                    :precision="2"
                    :show-button="false"
            ...
            <n-grid :cols="2" :x-gap="16">
              <n-gi>
                <n-form-item label="监控窗口" path="monitorWindow">
                  <n-input-number
                    v-model:value="formData.monitorWindow"
                    :min="60"
                    :max="3600"
                    :step="60"
                    :show-button="false"
              ...
              <n-gi>
                <n-form-item label="波动阈值" path="volatilityThreshold">
                  <n-input-number
                    v-model:value="formData.volatilityThreshold"
                    :min="0.1"
                    :max="500"
                    :precision="2"
                    :show-button="false"
            ...
            <n-grid :cols="3" :x-gap="16">
              ...
              <n-gi>
                <n-form-item label="启动止盈" path="autoStartRetreatPercent">
                  <n-input-number
                    v-model:value="formData.autoStartRetreatPercent"
                    :min="0.5"
                    :max="100"
                    :precision="2"
                    :show-button="false"
              ...
              <n-gi>
                <n-form-item label="止盈回撤" path="profitRetreatPercent">
                  <n-input-number
                    v-model:value="formData.profitRetreatPercent"
                    :min="5"
                    :max="100"
                    :precision="2"
                    :show-button="false"
...
<script lang="ts" setup>
  ...
  function initAllStrategies() {
    ...
    dialog.warning({
      title: '批量初始化策略',
      content:
        '将为此模板创建12种策略组合（4种市场状态 × 3种风险偏好），已存在的组合将跳过。是否继续？',
      positiveText: '确定',
      negativeText: '取消',
  ...
  function editStrategy(strategy: any) {
    ...
    try {
      configJson = strategy.configJson
        ? typeof strategy.configJson === 'string'
    ...
    formData.value = {
      ...
      volatilityThreshold:
        parseFloat(strategy.volatilityThreshold) || configJson.volatilityThreshold || 100,
      ...
      marginPercent:
        ...
        configJson.marginPercent ||
        parseFloat(strategy.marginPercentMin) ||
        12,
      ...
      autoStartRetreatPercent:
        parseFloat(strategy.autoStartRetreatPercent) || configJson.autoStartRetreatPercent || 3,
      profitRetreatPercent:
        parseFloat(strategy.profitRetreatPercent) || configJson.profitRetreatPercent || 25,
  ...
  async function handleSubmit(confirmed = false) {
    ...
    try {
      ...
      const data = editingStrategy.value
        ? { ...submitData, id: editingStrategy.value.id, confirmed }
        : submitData;
