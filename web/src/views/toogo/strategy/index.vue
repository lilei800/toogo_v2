                  ...
                  <n-gi v-for="group in myStrategies" :key="group.id">
                    <n-card hoverable size="small" class="strategy-card">
                      ...
                      <template #header-extra>
                        <n-dropdown
                          :options="getCardActions(group)"
                          @select="(key) => handleAction(key, group)"
                        >
                      ...
                      <n-space vertical :size="8">
                        <n-space align="center" :size="8">
                          <n-tag type="info" size="small">{{
                            getExchangeLabel(group.exchange)
                          }}</n-tag>
                          <n-tag size="small">{{ group.symbol }}</n-tag>
                          <n-tag
                            :type="
                              (group.strategyCount || 0) === 12
                                ? 'success'
                                : (group.strategyCount || 0) > 0
                                ? 'warning'
                                : 'default'
                            "
                            size="small"
                        ...
                        <n-space :size="4">
                          <n-tag size="tiny" :bordered="false">{{
                            group.orderType === 'market' ? '市价单' : '限价单'
                          }}</n-tag>
                          <n-tag size="tiny" :bordered="false">{{
                            group.marginMode === 'isolated' ? '逐仓' : '全仓'
                          }}</n-tag>
                        </n-space>
                      ...
                      <template #action>
                        <n-space justify="space-between">
                          <n-space :size="8">
                            <n-button size="small" @click="viewStrategies(group)"
                              >查看策略</n-button
                            >
                            ...
                            <n-button
                              size="small"
                              type="warning"
                              @click="openInitModal(group)"
                              v-if="(group.strategyCount || 0) < 12"
                            >
                              初始化
                            </n-button>
                          </n-space>
                          <n-button
                            type="primary"
                            size="small"
                            @click="createRobotWithStrategy(group)"
                          >
                            使用此策略
                          </n-button>
                        </n-space>
      ...
      >
        <n-form-item label="策略组名称" path="groupName">
          <n-input
            ...
            placeholder="如：BTC-USDT 高频策略 V1.0"
            maxlength="50"
          />
        </n-form-item>
        <n-form-item label="策略组标识" path="groupKey">
          <n-input
            ...
            placeholder="唯一标识，如：my_btc_usdt_v1（留空自动生成）"
...
<script setup lang="ts">
  ...
  const getCardActions = (group: any) => [
    ...
    { type: 'divider', key: 'd1' },
    {
      label: group.isDefault ? '取消默认' : '设为默认',
      key: 'default',
      icon: () => h('span', {}, '⭐'),
    },
    { type: 'divider', key: 'd2' },
  ...
  const getExchangeLabel = (exchange: string) => {
    ...
    return map[exchange] || exchange || 'Binance';
  };
