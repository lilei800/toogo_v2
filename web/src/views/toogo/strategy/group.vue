<template>
  <div class="strategy-group">
    ...
    <n-card class="guide-card" :bordered="false" style="margin-bottom: 16px">
      <n-space align="center" justify="space-between">
        ...
        <n-button type="primary" size="large" @click="quickCreateRobot">
          <template #icon><n-icon><RocketOutlined /></n-icon></template>
          一键创建机器人（使用官方策略）
        </n-button>
      </n-space>
    </n-card>
    ...
    <!-- 官方推荐策略区域 -->
    <n-card title="🔥 官方推荐策略" :bordered="false" style="margin-bottom: 16px" class="official-section">
          ...
          <n-card hoverable class="official-card">
            ...
            <n-space vertical :size="8">
              <n-text depth="3" style="font-size: 13px">
                {{ group.description || '专业团队精心调优的策略组合，包含12种市场状态和风险偏好组合。' }}
              </n-text>
              
              <n-space>
                <n-tag :bordered="false" size="small">{{ getExchangeLabel(group.exchange) }}</n-tag>
                <n-tag :bordered="false" size="small" type="success">{{ group.strategyCount || 12 }}种策略</n-tag>
              </n-space>
            </n-space>
    ...
    <n-card title="📋 我的策略模板" :bordered="false" class="proCard">
      ...
      <n-space style="margin-bottom: 16px">
        <n-select v-model:value="filterExchange" :options="exchangeOptions" placeholder="交易平台" clearable style="width: 140px" />
        <n-input v-model:value="filterSymbol" placeholder="搜索交易对" clearable style="width: 160px" />
    ...
    <n-modal v-model:show="showInitModal" preset="card" title="初始化策略" style="width: 500px">
      <n-alert type="info" style="margin-bottom: 16px">
        将为模板 <strong>{{ initGroup?.groupName }}</strong> 自动创建12种策略组合（4种市场状态 × 3种风险偏好）
      </n-alert>
      <n-space vertical>
    ...
    <n-modal v-model:show="showUseModal" preset="card" title="使用官方策略" style="width: 550px">
      ...
      <n-space vertical :size="12">
        ...
        <n-radio-group v-model:value="useMode">
          <n-space vertical>
            ...
            <n-radio value="apply" :disabled="robotOptions.length === 0">
              <n-space align="center">
                <span>应用到已有机器人</span>
                <n-text v-if="robotOptions.length === 0" depth="3" style="font-size: 12px">（暂无可用机器人）</n-text>
              </n-space>
            </n-radio>
          </n-space>
        </n-radio-group>
        ...
        <n-form-item v-if="useMode === 'apply'" label="选择机器人" style="margin-top: 8px">
          <n-select v-model:value="selectedRobotId" :options="robotOptions" placeholder="请选择机器人" />
        </n-form-item>
      </n-space>
...
import { 
  ...
  SettingOutlined, UnorderedListOutlined, BulbOutlined, 
...
// 计算属性：分离官方和用户策略
const officialGroups = computed(() => groupList.value.filter(g => g.isOfficial));
const userGroups = computed(() => groupList.value.filter(g => !g.isOfficial));
...
const columns = [
  ...
  },
  {
    title: '策略数',
    key: 'strategyCount',
    render: (row: any) => h(NTag, { type: row.strategyCount >= 12 ? 'success' : 'warning', size: 'small' }, () => `${row.strategyCount || 0}种`),
  },
  {
    title: '状态',
    key: 'isActive',
    render: (row: any) => h(NTag, { type: row.isActive ? 'success' : 'default', size: 'small' }, () => row.isActive ? '启用' : '禁用'),
  },
  { title: '排序', key: 'sort', width: 80 },
  {
    ...
    render: (row: any) =>
      h(NSpace, {}, () => [
        h(NButton, { size: 'small', type: 'primary', onClick: () => viewStrategies(row) }, { default: () => '查看策略', icon: () => h(NIcon, null, () => h(UnorderedListOutlined)) }),
        row.strategyCount < 12 && h(NButton, { size: 'small', type: 'warning', onClick: () => openInitModal(row) }, { default: () => '初始化', icon: () => h(NIcon, null, () => h(SettingOutlined)) }),
        h(NButton, { size: 'small', quaternary: true, onClick: () => handleEdit(row) }, { icon: () => h(NIcon, null, () => h(EditOutlined)) }),
...
function quickCreateRobot() {
  // 找到官方BTC-USDT策略
  const btcGroup = officialGroups.value.find(g => g.symbol === 'BTC-USDT') || officialGroups.value[0];
  if (btcGroup) {
    router.push({ 
...
async function confirmUseOfficial() {
  if (useMode.value === 'create') {
    ...
    try {
      ...
      // 优先选择平衡型-趋势市场策略
      const balancedStrategy = strategies.find((s: any) => s.riskPreference === 'balanced' && s.marketState === 'trend') || strategies[0];
...
async function handleDelete(row: any) {
  try {
    await http.request({ url: '/trading/strategy/group/delete', method: 'post', data: { id: row.id } });
...
async function handleSubmit() {
  ...
  try {
    const url = editingGroup.value ? '/trading/strategy/group/update' : '/trading/strategy/group/create';
