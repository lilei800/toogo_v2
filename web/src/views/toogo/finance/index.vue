        ...
        <n-card class="account-card balance-card">
          <div class="card-content">
            ...
            <div class="card-info">
              ...
              <div class="card-value">{{ formatNumber(accountInfo.balance) }} <span class="unit">USDT</span></div>
        ...
        <n-card class="account-card power-card">
          <div class="card-content">
            ...
            <div class="card-info">
              ...
              <div class="card-value">{{ formatNumber(accountInfo.power) }} <span class="unit">算力</span></div>
            </div>
          </div>
        </n-card>
      </n-gi>
        ...
        <n-card class="account-card gift-card">
          <div class="card-content">
            ...
            <div class="card-info">
              ...
              <div class="card-value">{{ formatNumber(accountInfo.giftPower) }} <span class="unit">积分</span></div>
        ...
        <n-card class="account-card commission-card">
          <div class="card-content">
            ...
            <div class="card-info">
              ...
              <div class="card-value">{{ formatNumber(accountInfo.commission) }} <span class="unit">USDT</span></div>
              <div class="card-actions">
      ...
      <div class="withdraw-modal">
        <n-form :model="withdrawForm" label-placement="left" label-width="80">
          <n-form-item label="提现账户">
            ...
            <span style="margin-left: 10px; color: #999;">
              可提现: {{ formatNumber(withdrawForm.fromAccount === 'balance' ? accountInfo.balance : accountInfo.commission) }} USDT
            </span>
          </n-form-item>
          <n-form-item label="提现金额">
            <n-input-number
              ...
              :min="10"
              :max="withdrawForm.fromAccount === 'balance' ? accountInfo.balance : accountInfo.commission"
              :precision="2"
              placeholder="最低10 USDT"
              style="width: 100%"
            />
        ...
        <n-alert type="warning" title="提现须知">
          <ul style="margin: 0; padding-left: 16px;">
            ...
            <li>实际到账：{{ formatNumber(withdrawForm.amount * (1 - accountInfo.withdrawFee / 100)) }} USDT</li>
          </ul>
        </n-alert>
      </div>
    </n-modal>
        ...
        <n-form :model="transferForm" label-placement="left" label-width="80">
          <n-form-item label="转出账户">
            <n-radio-group v-model:value="transferForm.fromAccount">
              <n-space>
                <n-radio value="balance">余额账户 ({{ formatNumber(accountInfo.balance) }} USDT)</n-radio>
                <n-radio value="commission">佣金账户 ({{ formatNumber(accountInfo.commission) }} USDT)</n-radio>
              </n-space>
            </n-radio-group>
          </n-form-item>
...
const rechargeRecords = computed(() => records.value.filter((r) => r.changeType === 'deposit' || r.changeType === 'admin_recharge'));
const withdrawRecords = computed(() => records.value.filter((r) => r.changeType?.startsWith('withdraw')));
const subscribeRecords = computed(() => records.value.filter((r) => r.changeType === 'subscribe' || r.changeType === 'subscribe_deduct'));
const transferRecords = computed(() => records.value.filter((r) => r.changeType?.startsWith('transfer')));
const powerRecords = computed(() => records.value.filter((r) => r.changeType === 'power_consume'));
