  ...
  <div class="commission-page">
    ...
    <n-grid cols="1 s:2 m:2 l:4 xl:4 2xl:4" :x-gap="12" :y-gap="12" responsive="screen">
      <n-gi>
        <n-card :bordered="false">
          <n-statistic label="今日佣金" :value="stat.todayCommission?.toFixed(4) || '0.0000'" suffix="USDT" />
        </n-card>
      </n-gi>
      <n-gi>
        <n-card :bordered="false">
          <n-statistic label="本周佣金" :value="stat.weekCommission?.toFixed(4) || '0.0000'" suffix="USDT" />
        </n-card>
      </n-gi>
      <n-gi>
        <n-card :bordered="false">
          <n-statistic label="本月佣金" :value="stat.monthCommission?.toFixed(4) || '0.0000'" suffix="USDT" />
        </n-card>
      </n-gi>
      <n-gi>
        <n-card :bordered="false">
          <n-statistic label="累计佣金" :value="stat.totalCommission?.toFixed(4) || '0.0000'" suffix="USDT" />
        </n-card>
      </n-gi>
    </n-grid>
    ...
    <n-card title="佣金分类" :bordered="false" class="proCard mt-3">
      <n-grid cols="1 s:2 m:2 l:2 xl:2 2xl:2" :x-gap="12" :y-gap="12" responsive="screen">
        <n-gi>
          <n-statistic label="邀请奖励" :value="stat.inviteReward?.toFixed(2) || '0.00'" suffix="积分">
            <template #prefix>
        ...
        <n-gi>
          <n-statistic label="订阅佣金" :value="stat.subscribeCommission?.toFixed(4) || '0.0000'" suffix="USDT">
            <template #prefix>
    ...
    <n-card title="佣金明细" :bordered="false" class="proCard mt-3">
      <template #header-extra>
        <n-space>
          <n-select v-model:value="searchParams.commissionType" :options="typeOptions" style="width: 120px" clearable placeholder="佣金类型" @update:value="loadList" />
        </n-space>
      </template>
      <n-data-table
        :columns="columns"
