<template>
  ...
  <n-modal v-model:show="resetModalVisible" preset="dialog" title="忘记密码" :mask-closable="true">
    ...
    <template #action>
      <n-space justify="end">
        <n-button @click="resetModalVisible = false" :disabled="resetSending">取消</n-button>
        <n-button type="primary" :loading="resetSending" @click="sendResetEmail">发送邮件</n-button>
      </n-space>
    </template>
  </n-modal>
</template>
...
<script lang="ts" setup>
  ...
  import { SendResetPwdEmail, SendSms } from '@/api/system/user';
  ...
  async function sendResetEmail() {
    const rule = { required: true, message: '请输入邮箱', trigger: 'blur' } as any;
    let hasError = false;
    validate.email(rule, resetEmail.value, (err?: Error) => {
      if (err) {
    ...
    try {
      await SendResetPwdEmail({ email: resetEmail.value });
      message.success('如果邮箱存在，重置链接已发送，请查收');
  ...
  async function handleLoginResp(request: Promise<any>) {
    ...
    try {
      ...
      message.destroyAll();
      console.error(e);
      // message.error(e.message || '登录失败'); // 拦截器可能已经提示了
      await refreshCode();
    } finally {
      loading.value = false;
    }
