<template>
  <div class="toogo-user-list">
    <n-space vertical>
      <!-- 搜索栏 -->
      <n-space>
        <n-input v-model:value="searchParams.username" placeholder="用户名" clearable style="width: 150px" @keyup.enter="loadData" />
        <n-select v-model:value="searchParams.vipLevel" :options="vipOptions" style="width: 120px" clearable placeholder="VIP等级" />
        <n-select v-model:value="searchParams.isAgent" :options="agentOptions" style="width: 120px" clearable placeholder="代理商" />
    ...
    <!-- 详情弹窗 -->
    <n-modal v-model:show="showDetailModal" title="Toogo用户详情" preset="card" style="width: 700px">
      <n-descriptions :column="2" label-placement="left" bordered v-if="currentRow">
        ...
        <n-descriptions-item label="VIP等级">
          <n-tag :type="currentRow.vipLevel > 5 ? 'warning' : 'info'">V{{ currentRow.vipLevel }}</n-tag>
        </n-descriptions-item>
        <n-descriptions-item label="是否代理商">
          <n-tag :type="currentRow.isAgent ? 'success' : 'default'">{{ currentRow.isAgent ? '是' : '否' }}</n-tag>
        </n-descriptions-item>
        <n-descriptions-item label="代理商等级">{{ currentRow.agentLevel || '-' }}</n-descriptions-item>
        ...
        <n-descriptions-item label="直推人数">{{ currentRow.inviteCount || 0 }}</n-descriptions-item>
        <n-descriptions-item label="团队人数">{{ currentRow.teamCount || 0 }}</n-descriptions-item>
        <n-descriptions-item label="消耗算力">{{ (currentRow.totalPowerConsume || 0).toFixed(2) }}</n-descriptions-item>
        <n-descriptions-item label="团队消耗">{{ (currentRow.teamTotalPowerConsume || 0).toFixed(2) }}</n-descriptions-item>
        <n-descriptions-item label="机器人限制">{{ currentRow.robotLimit || 1 }}</n-descriptions-item>
        <n-descriptions-item label="算力折扣">{{ currentRow.powerDiscount || 100 }}%</n-descriptions-item>
    ...
    <!-- 调整机器人限制弹窗 -->
    <n-modal v-model:show="showRobotModal" title="调整机器人限制" preset="dialog" style="width: 400px">
...
const columns = [
  { title: 'ID', key: 'memberId', width: 80 },
  {
    ...
    render: (row: any) => h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' } }, [
      h(NAvatar, { src: row.avatar, round: true, size: 32, fallbackSrc: 'https://via.placeholder.com/32' }),
  ...
  },
  {
    ...
    key: 'vipLevel',
    width: 90,
    render: (row: any) => h(NTag, { type: row.vipLevel > 5 ? 'warning' : row.vipLevel > 0 ? 'info' : 'default', size: 'small' }, 
  ...
  },
  {
    ...
    key: 'isAgent',
    width: 80,
    render: (row: any) => h(NTag, { type: row.isAgent ? 'success' : 'default', size: 'small' }, 
  ...
  },
  {
    ...
    key: 'actions',
    width: 100,
    render: (row: any) => h(NButton, { size: 'small', type: 'primary', onClick: () => viewDetail(row) }, { default: () => '详情' }),
  },
];
